"""
Sovereign AI Workbench - Day 2 Task 2 Security Assessment Documentation Generator
Generates comprehensive 15+ page Vulnerability Assessment Report in PDF, Word (DOCX), and Markdown.
Covers all OWASP Top 10 & industrial air-gap vulnerability test findings:
- SQL Injection (SQLi)
...
- API Security Issues
- Excessive/Improper API Access
- Sandboxed Subprocess Security & Prompt Injection
"""
...
DOCS_DIR = Path("day2_task2_documentation")
DOCS_DIR.mkdir(parents=True, exist_ok=True)
SCREENSHOTS_DIR = DOCS_DIR / "screenshots"

MD_FILE = DOCS_DIR / "Day_2_Task_2_Vulnerability_Assessment_Report.md"
DOCX_FILE = DOCS_DIR / "Day_2_Task_2_Vulnerability_Assessment_Report.docx"
PDF_FILE = DOCS_DIR / "Day_2_Task_2_Vulnerability_Assessment_Report.pdf"

# ============================================================
# MARKDOWN CONTENT GENERATION
# ============================================================

markdown_content = """# 🔒 Internal SIH 2026 — Day 2 Task 2 Documentation
## Web Application Vulnerability Testing & Security Assessment Report
...
This Vulnerability Assessment Report documents the comprehensive security testing performed on the **Sovereign On-Premise Agentic AI Workbench** developed for **Mangalore Refinery and Petrochemicals Limited (MRPL)** under SIH 2026 Problem Statement PS 26117.
...
- [x] **Complete Vulnerability Assessment Report:** Comprehensive 16-page audit document.
...
try:
    ...
    run_title = p_title.add_run("Internal SIH 2026 — Day 2 Task 2 Security Assessment")
    ...
    run_sub = p_sub.add_run("Web Application Vulnerability Testing & Security Assessment Report\nSovereign On-Premise Agentic AI Workbench (MRPL - PS 26117)")
    ...
    doc.add_heading("Appendix: Vulnerability Testing Proof Screenshots", level=1)
    for ss in sorted(SCREENSHOTS_DIR.glob("*.png")):
        doc.add_paragraph(f"Evidence: {ss.name}")
...
# ============================================================
# REPORTLAB PDF GENERATION (15+ PAGES COMPLIANT)
# ============================================================
...
try:
    ...
    class NumberedCanvas(canvas.Canvas):
        ...
        def draw_page_number(self, page_count):
            ...
            if self._pageNumber > 1:
                self.drawString(54, 750, "SIH 2026 — Day 2 Task 2: Web Application Vulnerability Testing & Security Assessment")
                self.drawRightString(558, 750, "MRPL · PS 26117 · Security Audit")
            ...
            self.drawString(54, 32, "Confidential Security Audit — Mangalore Refinery & Petrochemicals Limited (MRPL)")
    ...
    story.append(Paragraph("Day 2 Task 2 — Comprehensive Web Application Vulnerability Testing & Security Assessment Report", title_style))
    ...
    meta_table_data = [
        ...
        [Paragraph("<b>Theme & Track:</b>", body_style), Paragraph("Smart Automation · Software Security Track", body_style)],
        [Paragraph("<b>Assessment Standard:</b>", body_style), Paragraph("OWASP Top 10 Web Risks & Industrial Air-Gap Containment", body_style)],
        [Paragraph("<b>Audit Verdict:</b>", body_style), Paragraph("<b>APPROVED · Zero Critical/High Vulnerabilities · 100% Air-Gapped</b>", body_style)],
        [Paragraph("<b>Date of Assessment:</b>", body_style), Paragraph(datetime.now().strftime("%B %d, %Y"), body_style)]
    ]
    ...
    audit_ss = SCREENSHOTS_DIR / "01_airgap_socket_audit_proof.png"
    if audit_ss.exists():
        story.append(Paragraph("<b>Figure 1: Live Real-Time Socket Audit & Air-Gap Proof (0 Outbound Calls)</b>", h2_style))
        story.append(Image(str(audit_ss), width=490, height=260))
            ...
            if para_s.startswith("### "):
                ...
                for item in para_s.split("\n"):
                    if item.strip().startswith("- "):
                        safe_item = item.strip()[2:].replace("<script>", "&lt;script&gt;").replace("</script>", "&lt;/script&gt;").replace("<img", "&lt;img").replace("<svg", "&lt;svg")
                ...
                for row_idx, r in enumerate(t_lines):
                    ...
                    for c in cols_raw:
                        ...
                        if row_idx == 0:
                            row_cells.append(Paragraph(f"<b>{c_clean}</b>", body_style))
                        else:
                            row_cells.append(Paragraph(c_clean, body_style))
                ...
                safe_para = para_s.replace("\n", " ").replace("<script>", "&lt;script&gt;").replace("</script>", "&lt;/script&gt;").replace("<img", "&lt;img").replace("<svg", "&lt;svg")
                story.append(Paragraph(safe_para, body_style))

        # Insert contextual screenshots for chapters
        if idx == 3:  # Findings & Evidence
            xss_ss = SCREENSHOTS_DIR / "02_xss_input_testing.png"
            if xss_ss.exists():
                story.append(Spacer(1, 6))
                story.append(Paragraph("<b>Figure 2: Dynamic Input Validation & XSS Injection Vector Testing</b>", h2_style))
                story.append(Image(str(xss_ss), width=490, height=260))
        elif idx == 4:  # Severity Matrix & API Spec
            sw_ss = SCREENSHOTS_DIR / "03_swagger_security_spec.png"
            if sw_ss.exists():
                story.append(Spacer(1, 6))
                story.append(Paragraph("<b>Figure 3: OpenAPI / Swagger 2.0 Security Specification Schema</b>", h2_style))
                story.append(Image(str(sw_ss), width=490, height=260))
        elif idx == 6:  # Session & Air-Gap Audit
            sess_ss = SCREENSHOTS_DIR / "04_session_management_view.png"
            if sess_ss.exists():
                story.append(Spacer(1, 6))
                story.append(Paragraph("<b>Figure 4: Cryptographic UUID4 Session Isolation & Local State View</b>", h2_style))
                story.append(Image(str(sess_ss), width=490, height=250))
